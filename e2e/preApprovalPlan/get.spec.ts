...
describe('Testing pre approval plan, create', () => {
	test('should make a POST request with the correct parameters', async () => {
		...
		const getPlan = await preApprovalPlan.get({ preApprovalPlanId: createPlan.id });
		expect(getPlan.reason).toBe(body.reason);
		expect(getPlan.auto_recurring.frequency).toBe(body.auto_recurring.frequency);
		expect(getPlan.auto_recurring.frequency_type).toBe(body.auto_recurring.frequency_type);
		expect(getPlan.auto_recurring.transaction_amount).toBe(body.auto_recurring.transaction_amount);
		expect(getPlan.auto_recurring.currency_id).toBe(body.auto_recurring.currency_id);
		expect(getPlan.back_url).toBe(body.back_url);
		expect(getPlan).toEqual(expect.objectContaining({
