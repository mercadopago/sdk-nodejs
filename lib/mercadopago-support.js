...
module.exports = function () {
  ...
  function getRest(uri) {
    ...
    callback = (arguments[arguments.length - 1] !== undefined && typeof arguments[arguments.length - 1] === 'function')
   ...
   * @param callback
   */
  function searchPayment(filters, offset, limit, callback) {
    showWarning();

    if (!isNaN(offset)) filters.offset = offset;
    if (!isNaN(limit)) filters.limit = limit;

    return paymentModule.search({
      qs: filters
    }, callback);
  }
