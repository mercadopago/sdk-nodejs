        ...
        return new Promise(function(resolve, reject){
            ...
            if( options.method == 'GET' || options.method == 'DELETE' ){
                ...
                if( totalParams > _arguments.length ){
                    var error = new Error('Expecting parameters: ' + pathParameters.join(', ').replace(/\:/g, '') + '');
                    reject(error);
                ...
                if( missingProperties.length > 0 ){
                    var error = new Error('The JSON is missing the following properties: ' + missingProperties.join(', '));
                    reject(error);
                    return callback.apply(null, [error, null]);
                }
            }
            ...
            var schema = me.schema,
                needIdempotency = me.idempotency ? true : false;
            ...
            return _this.generateAccessToken().then(function(accessToken){
                return requestManager.exec({
                    schema: schema, //Schema to validate if not GET or DELETE
...
requestManager.getUserCredentials = function(clientSecret, authorizationCode, redirectURI, callback){
    ...
    return new Promise(function(resolve, reject){
        me.exec({
            ...
            if(err){
                var mpError = new Error('Error getting user credentials: ' + err.message);
                reject(mpError);
                return callback.apply(null, [mpError, null]);
            }else{
                resolve(response);
...
requestManager.buildRequest = function(options){
    ...
    request.uri = (options.base_url) ? options.base_url + options.path : configurations.getBaseUrl() + options.path;
    ...
    request.qs = (options.config && options.config.qs) ? options.config.qs : {}; //Always set the querystring object
