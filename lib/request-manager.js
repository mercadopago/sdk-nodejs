...
requestManager.exec = function(options, callback){
    ...
    request(req, function(error, response, body){
        if( error ) {
            ...
            var parseBody = me.parseJSON(body);

            if( response.statusCode < 200 || response.statusCode >= 300 ){
                return callback.apply(null, [new Error(parseBody ? parseBody.message : 'Unknown Error'), null]);
            }else{
                if(parseBody){
                    ...
                    return callback.apply(null, [new Error('Error parsing JSON response from Mercadopago API'), null]);
                }
            }
        }
    });
...
/**
 * Returns the parsed object if it can
 * @param body
 * @param errorResponse
 * @returns {*}
 */
requestManager.parseJSON = function(body, errorResponse){
    try{
