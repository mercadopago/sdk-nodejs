  ...
  return function () {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (options.method === 'GET' && config.cache) {
        hash = cacheManager.generateCacheKey(options.path, options.headers, config.qs);

        // If it is on cache return the value
        if (cacheManager.has(hash)) {
          cachedResponse = cacheManager.get(hash);
          resolve(cachedResponse);
          return callback.apply(null, [null, cachedResponse]);
        }
      }
  ...
  return new Promise(function (resolve, reject) {
    ...
    return request(req, function (error, response, body) {
      ...
      if (response.statusCode < 200 || response.statusCode >= 300) {
        mpError = new MercadoPagoError(body.message, response.statusCode, req.headers['x-idempotency-key']);
        reject(mpError);
      ...
      if (options.method === 'GET' && options.cache) {
        cacheManager.save(cacheManager.generateCacheKey(options.path, options.headers, ((options.config) ?
          options.config.qs : {})), mpResponse, response.headers['cache-control']);
      }
