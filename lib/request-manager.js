    ...
    if( configurations.getAccessToken() ){
        ...
        this.exec({
            ...
            if(err){
                ...
                configurations.setAccessToken(response.access_token);
                configurations.setRefreshToken(response.refresh_token);
                return callback.apply(null, [null, response.access_token]);
            }
        });
    }
};
    ...
    request(req, function(error, response, body){
        if( error ) {
            ...
            if( response.statusCode < 200 || response.statusCode >= 300 ){
                ...
                return callback.apply(null, [null, body]);
            }
        }
    });
};