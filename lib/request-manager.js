...
requestManager.describe = function (options) {
  ...
  return function () {
    ...
    return new Promise(function (resolve, reject) {
      ...
      var pathParameters = mainContext.getPathParamsKeyNames(options.path);
      var missingPayloadProperties = []; // Stores the missing payload path params (if there is any). POST, PUT, PATCH
      var schema = me.schema; // Schema from resource
      var needIdempotency = !!me.idempotency; // Idempotency from resource
      var config = {};
      var payload = {};
      var error;
  ...
  return new Promise(function (resolve, reject) {
    ...
    return me.REST_CLIENT(req, function (error, response, body) {
      if (error) {
        ...
        mpError = new MercadoPagoError(error.message, null, null, req.headers['x-idempotency-key'], options, me);
        reject(mpError);
